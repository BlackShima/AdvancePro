package se233.chapter6_ex1.controller;
...
import se233.chapter6_ex1.Launcher;
import se233.chapter6_ex1.model.character.BasedCharacter;
import se233.chapter6_ex1.model.character.BattleMageCharacter;
import se233.chapter6_ex1.model.item.Armor;
import se233.chapter6_ex1.model.item.BasedEquipment;
...
public class AllCustomHandler {
    ...
    public static void onDragOver(DragEvent event, String slotType) {
        ...
        if ("Weapon".equals(slotType) && eq instanceof Weapon) {
            if (character instanceof se233.chapter6_ex1.model.character.BattleMageCharacter || ((Weapon) eq).getDamageType() == character.getType()) {
                allowed = true;
            }
        }
        else if ("Armor".equals(slotType) && eq instanceof Armor) {
            if (!(character instanceof se233.chapter6_ex1.model.character.BattleMageCharacter)) {
                allowed = true;
            }
        }
        if (allowed) {
    ...
    public static void onDragDropped(DragEvent event, Label lbl, StackPane imgGroup) {
        ...
        if (dragboard.hasContent(BasedEquipment.DATA_FORMAT)) {
            ...
            logger.info("Equipped item: {}", retrieved.getName());
